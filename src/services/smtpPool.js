...
// Signature DKIM (délivrabilité) — appliquée à tous les transporteurs si
// DKIM_DOMAIN + DKIM_PRIVATE_KEY sont posés. Dormant sinon.
function dkimOption() {
  const { domain, selector, privateKey } = config.features.dkim;
  if (!domain || !privateKey) return undefined;
  return {
    domainName:  domain,
...
class SmtpPool {
  ...
  constructor(configs) {
    ...
    for (const cfg of configs) {
      ...
      const t = nodemailer.createTransport({
        ...
        maxConnections: 3,
        rateLimit: 5,
        dkim:   dkimOption(),   // signe les messages si DKIM configuré
      });
  ...
  async send(mailOptions, _depth = 0) {
    if (!this.#transporters.length) {
      throw new Error('Aucun transporteur SMTP configuré');
    }
    // Guard anti-récursion infinie : max 3 tentatives de failover
    if (_depth >= 3) {
    ...
    try {
      ...
      if (code === 535 || code === 550 || code === 421) {
        ...
        return this.send(mailOptions, _depth + 1); // profondeur bornée
      }
      throw err;
    }
  }
