...
export class Queue {
  ...
  // UPDATE…RETURNING = sélection + verrouillage en une instruction atomique :
  // plusieurs PROCESSUS workers peuvent partager la même DB (WAL) sans
  // jamais traiter deux fois le même job.
  #claim(limit) {
  ...
  /**
   * Remet en file les jobs de CE type restés en 'processing' après un crash.
   * Chaque process worker n'appelle ceci que pour les types qu'il traite —
   * jamais pour ceux d'un autre process (sinon on volerait ses jobs en cours).
   */
  recoverZombies() {
