...
router.get('/', async (req, res, next) => {
  try {
    ...
    const totalLeads = db.prepare(
      'SELECT COUNT(*) as n FROM leads WHERE user_id = ?'
    ).get(userId).n;
    ...
    const totalViews = db.prepare(
      'SELECT COALESCE(SUM(views), 0) as n FROM sites WHERE user_id = ?'
    ).get(userId).n;
    ...
    // Taux de conversion : leads avec site / total leads
    ...
    // Sites avec vues récentes (actifs = vus dans les 7 derniers jours)
    const activeSites = db.prepare(`
      SELECT COUNT(*) as n FROM sites
      WHERE user_id = ? AND last_viewed >= datetime('now', '-7 days')
    `).get(userId).n;

    // Top 5 sites les plus vus
    const topSites = db.prepare(`
      SELECT s.id, s.slug, s.url, s.views, s.last_viewed,
             l.name as lead_name, l.city, l.email as lead_email
      FROM sites s
    ...
    // Activité des 7 derniers jours (leads créés par jour)
